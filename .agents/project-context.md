...
- AI 文本记账。
- AI 票据识别。
- 后续 AI 财务问答与洞察：用户可用自然语言询问账本统计，AI 必须通过 NestJS 受控工具查询已授权数据，不能直接查库或绕过权限。
...
- `.agents/` 已成为跨工具共享规则源，`.codex/`、`CLAUDE.md`、`GEMINI.md` 和 `.cursor/rules/` 保留为适配入口；当前推荐 Agent skills 已安装到 `.agents/skills/`，`.agents/skills.md` 记录调用时机和跨工具降级规则；`.agents/checklists/startup.md` 和 `.agents/checklists/handoff.md` 已用于替代每轮重复粘贴长提示词。根目录提供 `verify:admin-web`、`verify:api`、`verify:scripts`、`verify:workspace-light` 和 `e2e:m4:ai-text`，并已提供轻量 `.githooks/pre-commit`。本地系统管理员账号可通过 `apps/api` 的 `admin:bootstrap` 脚本创建或提权。
...
- 项目推荐 skills 已固定在 `.agents/skills/`，后续对项目生效的 skill 更新应优先修改仓库级副本并同步 `.agents/skills.md`。
...
- 以上技能只作为框架实践辅助；项目边界仍以 `AGENTS.md`、`.agents/development-rules.md` 和中文模块文档为准。
