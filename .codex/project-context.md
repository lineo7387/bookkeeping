...
- `apps/admin-web` 已创建，当前为 Vue 3、TypeScript、Vite、Tailwind CSS、`lucide-vue-next`，已通过 `@bookkeeping/api-client` 接入 NestJS 登录接口和 M3/M4 后台只读 Admin API；系统管理员登录后 access token 写入 Pinia 持久化会话。本地 Vite dev server 已配置 `/api` 代理到 `http://127.0.0.1:3000`。
...
- `apps/mobile` 尚未创建。
- M2 账户余额流水联动已完成；M3 基础统计 API、审计日志基础写入与业务接入能力、后台只读 Admin API 基础能力和后台 Web 真实 Admin API 接入已完成首版。后台 API 当前包含 `users.is_system_admin`、`SystemAdminGuard`、用户列表、账本列表、真实 AI 任务摘要列表和审计日志查询；账本、成员、账户、分类和流水成功写操作已写入审计日志；后台 Web 提供系统管理员登录页，并展示这些只读接口的首屏分页样本。
