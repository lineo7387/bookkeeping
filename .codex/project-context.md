...
- `apps/api` 已创建，当前包含 NestJS 11、Prisma 7、`@nestjs/config`、PrismaService、认证与会话、JWT Guard、用户模块、账本模块、成员角色管理、Ledger Policy 层、账户、分类、流水 API、M2 账户余额流水联动、基础统计 API、审计日志基础写入与业务接入能力，以及 M3 后台只读 Admin API 基础能力。
...
- `packages/api-client` 已创建，只封装前端访问 NestJS API 的轻量请求能力，当前包含 AI 候选相关入口和 M3 Admin 只读列表入口。
- `apps/mobile` 尚未创建。
- M2 账户余额流水联动已完成；M3 基础统计 API、审计日志基础写入与业务接入能力、后台只读 Admin API 基础能力和后台 Web 真实 Admin API 接入已完成首版。后台 API 当前包含 `users.is_system_admin`、`SystemAdminGuard`、用户列表、账本列表、AI 任务占位列表和审计日志查询；账本、成员、账户、分类和流水成功写操作已写入审计日志；后台 Web 展示这些只读接口的首屏分页样本。严格按路线继续时，不提前实现 M4 AI 文本记账。
