...
- M2 账户余额流水联动已完成；M3 基础统计 API、审计日志基础写入与业务接入能力、后台只读 Admin API 基础能力已完成首版。后台 API 当前包含 `users.is_system_admin`、`SystemAdminGuard`、用户列表、账本列表、AI 任务占位列表和审计日志查询；账本、成员、账户、分类和流水成功写操作已写入审计日志；尚未接入后台 Web 真实 API。严格按路线继续时，下一步可进入 M3 后台 Web 真实 Admin API 接入，不提前实现 M4 AI 文本记账。
