...
- `apps/api` 已创建，当前包含 NestJS 11、Prisma 7、`@nestjs/config`、PrismaService、认证与会话、JWT Guard、用户模块、账本模块、成员角色管理、Ledger Policy 层、账户、分类、流水 API、M2 账户余额流水联动、基础统计 API，以及审计日志基础写入能力。
...
- M2 账户余额流水联动已完成；M3 基础统计 API 与审计日志基础写入能力已完成首版。审计日志目前只有 Prisma 模型、Repository、Service 和单元测试，尚未接入后台 Admin API 或具体业务敏感操作。严格按路线继续时，下一步应进入 M3 后台管理 API，不提前实现 M4 AI 文本记账。
