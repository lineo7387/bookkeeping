...
- `apps/api` 已创建，当前包含 NestJS 11、Prisma 7、`@nestjs/config`、PrismaService、认证与会话、JWT Guard、用户模块、账本模块、成员角色管理、Ledger Policy 层、账户、分类、流水 API、M2 账户余额流水联动、基础统计 API、审计日志基础写入能力，以及 M3 后台只读 Admin API 基础能力。
...
- M2 账户余额流水联动已完成；M3 基础统计 API、审计日志基础写入能力、后台只读 Admin API 基础能力已完成首版。后台 API 当前包含 `users.is_system_admin`、`SystemAdminGuard`、用户列表、账本列表、AI 任务占位列表和审计日志查询；尚未把具体业务敏感操作接入审计日志，也尚未接入后台 Web 真实 API。严格按路线继续时，下一步应继续 M3 审计日志业务接入，不提前实现 M4 AI 文本记账。
