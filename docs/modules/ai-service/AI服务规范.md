...
- 后续 AI 财务问答与洞察：基于 NestJS 受控工具查询结果组织答复，例如“这周消费了多少钱，主要用在什么地方”。该能力不能让 FastAPI/LLM 直接访问数据库或绕过权限。

AI 服务只提高录入效率和分析效率，不替代用户做最终业务决策。所有候选结果必须先由 NestJS 保存为 `ai_extractions`，再由用户确认后才能创建正式 `transaction`；财务问答中的金额和统计结论必须来自 NestJS 执行的真实查询结果。
...
- M4 当前对前端提供 `POST /ledgers/:ledgerId/ai/text-parse`、`GET /ledgers/:ledgerId/ai/tasks`、`GET /ai/tasks/:taskId`、`POST /ai/extractions/:extractionId/confirm`、`POST /ai/extractions/:extractionId/reject` 等接口。
- `POST /ledgers/:ledgerId/ai/receipt-ocr` 是后续 M5 票据 OCR 计划接口，不属于当前 M4 已实现范围。
...
- M4 文本解析候选的 `type` 只允许 `expense`、`income`；`transfer` 候选需要转账目标账户补全规则，留到后续扩展。
...
- M4 文本候选可返回 `missingFields` 和 `reviewMessage`。`missingFields` 只允许 `accountId`、`categoryId`，表示确认正式流水前必须由 NestJS 用户确认接口补齐的数据库字段；`reviewMessage` 是面向用户的简短中文提示。
...
- 如果候选结果置信度低于产品阈值但金额可识别，FastAPI 应优先返回低置信度候选，并通过 `missingFields` / `reviewMessage` 标记需要手动补全；如果金额不可识别或无法形成候选，再返回 `status = "failed"`。
...
- 低置信度候选：deterministic parser 已找到金额但未匹配到账本分类或账户提示。响应仍为 `status = "succeeded"`，并通过 `candidate.missingFields` 和 `candidate.reviewMessage` 提醒 NestJS/前端在确认前补齐正式字段。
...
- 增加 AI 财务问答与洞察：用户可询问“这周消费了多少钱，主要用在什么地方”等账本问题。该能力必须通过 NestJS 受控工具查询数据，FastAPI/LLM 不直接访问数据库，不绕过 Ledger Policy、私密流水可见性或统计口径。
- 为 AI 财务问答提供 intent 分类和工具调用契约：模型先区分账本分析、记账录入、应用帮助、通用知识和不支持问题；只有账本分析才申请受控工具。金额、分类占比、成员消费和时间范围必须来自 NestJS 执行的真实查询结果。
- 增加消费洞察、预算提醒和异常支出检测，这些能力应复用 AI 财务问答的工具调用与引用数据机制。
