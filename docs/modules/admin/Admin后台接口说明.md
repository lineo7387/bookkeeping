...
Admin 模块提供平台系统管理员排查问题所需的只读后台接口。M3 第一版只覆盖用户列表、账本列表、AI 任务占位列表和审计日志列表，不提供后台写操作；后台 Web 已通过 `@bookkeeping/api-client` 接入这些只读接口。

## 业务规则

- 后台接口只用于平台运营和问题排查，不替代普通用户账本协作接口。
- M3 阶段所有 Admin 接口均为只读接口。
- 用户列表不得返回 `password_hash`、refresh token hash 或其他认证密钥。
- 账本列表只返回账本摘要和成员数量，不返回私密流水、私密账户明细或交易明细。
- AI 任务列表在 M3 仅作为占位接口返回空列表；真实 AI 任务模型和任务编排属于 M4 后续范围。
...
查询 AI 任务摘要。M3 暂无 AI 任务数据模型，本接口返回空列表，用于固定后台 API 入口。
...
- M4 创建 AI 任务模型后，将 `/admin/ai/tasks` 改为真实只读查询。
