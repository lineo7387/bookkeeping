...
- 项目脚手架由用户执行创建命令，创建完成后由 Codex 负责配置、目录规范、文档模板和后续修改。
...
- FastAPI/LLM 只负责理解问题、选择工具和组织答复，不承担认证、权限、主业务状态或直接数据库访问。
...
创建框架时，Codex 只给出创建命令提示，由用户执行。用户执行完成后，Codex 再负责修改配置和补充规范。
